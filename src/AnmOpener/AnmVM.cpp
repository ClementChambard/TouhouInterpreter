...
{
    interrupt(i);
    for (AnmVMList_t* child = list_of_children.next; child != nullptr; child = child->next) {
        if (child->value)
...
AnmSprite AnmVM::getSprite() const { return AnmManager::loadedFiles[anm_loaded_index].getSprite(sprite_id); }
...
int AnmVM::update(bool /*printInstr*/)
{
    ...
    int8_t* instructions = AnmManager::loadedFiles[anm_loaded_index].getScript(script_id);
    ...
    if (pending_interrupt != 0) {
        ...
        while (instructions[offset] != -1) {
            ...
            if (instype == 5 && *reinterpret_cast<int32_t*>(&(instructions[offset + 8])) == pending_interrupt) {
    ...
    if (instr_offset >= 0) {
        ...
        while (instime <= time_in_script && (bitflags.activeFlags == ANMVM_ACTIVE || time_in_script == -1)) {
...
void AnmVM::getSpriteCorners2D(glm::vec2* corners)
{
    ...
    float b = t - 1;
    glm::mat4 rotate;
    pos4 = glm::vec4(pos4.x + entity_pos.x, -pos4.y + entity_pos.y, pos4.z + entity_pos.z, 0);
    pos4.z = 0;
    rotate = glm::rotate(glm::mat4(1.0f), -rotation.z, glm::vec3(0, 0, 1));
    corners[0] = { pos4 + rotate * glm::vec4(l * s.w * XS + anchor_offset.x, t * s.h * YS + anchor_offset.y, 0, 0) };
    corners[1] = { pos4 + rotate * glm::vec4(r * s.w * XS + anchor_offset.x, t * s.h * YS + anchor_offset.y, 0, 0) };
    corners[2] = { pos4 + rotate * glm::vec4(r * s.w * XS + anchor_offset.x, b * s.h * YS + anchor_offset.y, 0, 0) };
    corners[3] = { pos4 + rotate * glm::vec4(l * s.w * XS + anchor_offset.x, b * s.h * YS + anchor_offset.y, 0, 0) };
}
...
void AnmVM::destroy()
{
    if (id.val && (id.val & AnmID::fastIdMask) != AnmID::fastIdMask) {
        ...
        ListUtil::listInsertAfter(&AnmManager::freelist_head, &AnmManager::fastArray[id.val & AnmID::fastIdMask].freelistNode);
    }
...
void AnmVM::write_sprite_corners_2d(glm::vec4* corners)
{
    ...
    case 2:
    case 3:
        write_sprite_corners__without_rot(corners[0], corners[1], corners[2], corners[3]);
        break;
    case 1:
        write_sprite_corners__with_z_rot(corners[0], corners[1], corners[2], corners[3]);
        return;
    }
    return;
}

void AnmVM::write_sprite_corners__without_rot(glm::vec4& tl, glm::vec4& tr, glm::vec4& bl, glm::vec4& br)
{
    tl.x = bitflags.anchorX == 0 ? -0.5f : (bitflags.anchorX == 1 ? -1 : 0);
    tr.x = bitflags.anchorX == 0 ? 0.5f : (bitflags.anchorX == 1 ? 0 : 1);
    bl.x = bitflags.anchorX == 0 ? -0.5f : (bitflags.anchorX == 1 ? -1 : 0);
    br.x = bitflags.anchorX == 0 ? 0.5f : (bitflags.anchorX == 1 ? 0 : 1);
    tl.y = bitflags.anchorY == 0 ? 0.5f : (bitflags.anchorX == 2 ? 0 : 1);
    tr.y = bitflags.anchorY == 0 ? 0.5f : (bitflags.anchorX == 2 ? 0 : 1);
    bl.y = bitflags.anchorY == 0 ? -0.5f : (bitflags.anchorX == 2 ? -1 : 0);
    br.y = bitflags.anchorY == 0 ? -0.5f : (bitflags.anchorX == 2 ? -1 : 0);
    tl.x *= sprite_size.x;
    tl.y *= sprite_size.y;
    tr.x *= sprite_size.x;
    tr.y *= sprite_size.y;
...
void AnmVM::write_sprite_corners__with_z_rot(glm::vec4& tl, glm::vec4& tr, glm::vec4& bl, glm::vec4& br)
{
    tl.x = bitflags.anchorX == 0 ? -0.5f : (bitflags.anchorX == 1 ? -1 : 0);
    tr.x = bitflags.anchorX == 0 ? 0.5f : (bitflags.anchorX == 1 ? 0 : 1);
    bl.x = bitflags.anchorX == 0 ? -0.5f : (bitflags.anchorX == 1 ? -1 : 0);
    ...
    tl.x = tl.x * cos(rotation.z) - tl.y * sin(rotation.z);
    tl.y = tl.y * cos(rotation.z) + tl.x * sin(rotation.z);
    tr.x = tr.x * cos(rotation.z) - tr.y * sin(rotation.z);
    tr.y = tr.y * cos(rotation.z) + tr.x * sin(rotation.z);
    bl.x = bl.x * cos(rotation.z) - bl.y * sin(rotation.z);
    bl.y = bl.y * cos(rotation.z) + bl.x * sin(rotation.z);
    br.x = br.x * cos(rotation.z) - br.y * sin(rotation.z);
    br.y = br.y * cos(rotation.z) + br.x * sin(rotation.z);
    tl += glm::vec4(p, 0);
    tr += glm::vec4(p, 0);
    bl += glm::vec4(p, 0);
    br += glm::vec4(p, 0);
...
int AnmVM::write_sprite_corners__mode_4()
{
    ...
    // D3DXVec3Project(&local_74,&local_68,&(SUPERVISOR.current_camera)->viewport, &(SUPERVISOR.current_camera)->projection_matrix, &(SUPERVISOR.current_camera)->view_matrix,&local_50);
    // if (local_74.z < 0 || local_74.z > 1) return -1;
    // D3DXVec3Project(&zStack_5c,&(SUPERVISOR.current_camera)->right, &(SUPERVISOR.current_camera)->viewport, &(SUPERVISOR.current_camera)->projection_matrix, &(SUPERVISOR.current_camera)->view_matrix,&local_50);
    // dVar7 = math::point_distance(zStack_5c, local_74) * 0.5;
    ...
    // SPRITE_TEMP_BUFFER[0].transformed_pos.x += fStack_84 * cos(rotation.z) - fStack_88 * sin(rotation.z);
    // SPRITE_TEMP_BUFFER[1].transformed_pos.x += fStack_8c * cos(rotation.z) - fStack_88 * sin(rotation.z);
    // SPRITE_TEMP_BUFFER[2].transformed_pos.x += fStack_84 * cos(rotation.z) - fVar8 * sin(rotation.z);
    // SPRITE_TEMP_BUFFER[3].transformed_pos.x += fStack_8c * cos(rotation.z) - fVar8 * sin(rotation.z);
    // SPRITE_TEMP_BUFFER[0].transformed_pos.y += fStack_84 * sin(rotation.z) + fStack_88 * cos(rotation.z);
    // SPRITE_TEMP_BUFFER[1].transformed_pos.y += fStack_8c * sin(rotation.z) + fStack_88 * cos(rotation.z);
    // SPRITE_TEMP_BUFFER[2].transformed_pos.y += fStack_84 * sin(rotation.z) + fVar8 * cos(rotation.z);
    // SPRITE_TEMP_BUFFER[3].transformed_pos.y += fStack_8c * sin(rotation.z) + fVar8 * cos(rotation.z);
    return 0;
}

// TODO: set this
float SURF_ORIGIN_ECL_FULL_X = 0;
float SURF_ORIGIN_ECL_FULL_Y = 0;
float SURF_ORIGIN_ECL_X = 0;
float SURF_ORIGIN_ECL_Y = 0;
...
void AnmVM::transform_coordinate(glm::vec3& p)
{
    ...
    if (!__root_vm__or_maybe_not || bitflags.noParent) {
        /* if origin is not top left, don't change pos */
        if (bitflags.originMode != 0) {
            if (bitflags.originMode != 1) {
        ...
        if (bitflags.parRotate) {
            ...
            p.y = p.y * cos(__root_vm__or_maybe_not->rotation.z) + p.x * sin(__root_vm__or_maybe_not->rotation.z);
            p.x = p.x * cos(__root_vm__or_maybe_not->rotation.z) - p.y * sin(__root_vm__or_maybe_not->rotation.z);
        }
        pos += __root_vm__or_maybe_not->get_own_transformed_pos();
    }
}
...
int AnmVM::check_interrupt()
{
    ...
    // pzVar18 = NULL;
    // do {
    //   while (pzVar26 = anm_script, pzStack_118 = pzVar18, sVar5 = *(short *)&(pzVar26->prefix).interrupt_return_time.previous, sVar5 != 5)
    //   {
    //       if (sVar5 == -1) {
    //           goto PENDING_INTERRUPT_END;
    //       }
    //  IN_SEARCH_FOR_LABEL:
    //     uVar15 = (uint)*(ushort *) ((int)&(pzVar26->prefix).interrupt_return_time.previous + 2);
    //     pzVar18 = (zAnmVM *) ((int)&(pzStack_118->prefix).interrupt_return_time.previous + uVar15);
    //     anm_script = (zAnmVM *) ((int)&(pzVar26->prefix).interrupt_return_time.previous + uVar15);
    //   }
    ...
    //         pending_interrupt = 0;
    //         if (*(short *)&(pzVar24->prefix).interrupt_return_time.previous != 5) {
    //           if (pzStack_11c == NULL) {
    //             time_in_script--;
    ...
    //        (root_vm->prefix).interrupt_return_offset = (root_vm->prefix).instr_offset;
    //        uVar11 = *(ushort *)((int)&(pzVar24->prefix).interrupt_return_time.previous + 2);
    //        in_stack_fffffebc = CONCAT412(0x467007,time_in_script);
    //        time_in_script = pzVar24->interrupt_return_time.current;
    //        field_0x530 |= 1;
    //        instr_offset = (int32_t)((int)&(pzStack_118->prefix.interrupt_return_time).previous + (uint)uVar11);
    //        goto START_SWITCH;
    //  }
    ...
    //  ;
    //  pzVar18 = (zAnmVM *)
    //            ((int)&(pzStack_118->prefix).interrupt_return_time.previous + uVar15);
    //  anm_script = (zAnmVM *)
...
int AnmVM::run()
{
    ...
    if (instr_offset > -1 && !bitflags.f530_20) {
        ...
        if (pending_interrupt)
            check_interrupt();
        if (bitflags.activeFlags == 0b01 /*&& GAME_THREAD_PTR && (GAME_THREAD_PTR->flags & 2)*/) {
            GAME_SPEED = saved_game_speed;
            return 0;
        }
        // START_LOOP:
...
AnmID AnmVM::add_child(int i, int mode)
{
    ...
    auto new_vm = AnmManager::getVM(AnmManager::SpawnVM(anm_loaded_index, i, mode & 4, mode & 2));
    new_vm->bitflags.randomMode = 0b1;
    new_vm->bitflags.colorizeChildren = bitflags.colorizeChildren;
    new_vm->parent_vm = this;
    new_vm->__root_vm__or_maybe_not = __root_vm__or_maybe_not ? __root_vm__or_maybe_not : this;
    if (!new_vm->layer)
        new_vm->layer = layer;
    new_vm->update();
