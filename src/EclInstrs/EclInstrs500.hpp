...
int Enemy::execInstr(EclRunContext_t* cont, const EclRawInstr_t* instr)
{
    _insNop
        ...
        _ins(500, setHurtbox) _f(w) _f(h) _args enemy.hurtbox_size
        = { w, h }; // sets some vars and call a func, see at th17.exe::423af2
    ...
    _ins(504, moveLimit) _f(x) _f(y) _f(w) _f(h) _args enemy.moveLimitPos = { x, y };
    ...
    _ins(509, dropItems) _args if (GLOBALS.inner.SPELL_ID < 0) enemy.drops.eject_all_drops(enemy.final_pos.pos);
    ...
    _ins(512, setBoss) _S(a) _args if (a < 0) EnemyManager::GetInstance()->data.boss_ids[0] = 0;
    if (a >= 0)
        EnemyManager::GetInstance()->data.boss_ids[a] = enemyId;

    _ins(513, timerReset) _args _notImpl;
    _ins(514, setInterrupt) _S(slot) _S(hp) _S(t) _z(sub) _args enemy.interrupts[slot].life = hp;
    ...
    _ins(520, bossWait) _args if (isBoss_)
    {
        cont->time--;
        _ret;
    }
    _ins(521, setTimeout) _S(slot) _z(sub) _args enemy.interrupts[slot].subTimeout = sub;
    _ins(522, spellEx) _S(i) _S(t) _S(ty) _z(name) _args Spellcard::GetInstance()->Init(i, t, ty, name, EnemyManager::GetInstance()->getBoss(enemyId));
    ...
    _ins(537, spell) _S(i) _S(t) _S(ty) _z(name) _args Spellcard::GetInstance()->Init(i + GLOBALS.inner.DIFFICULTY, t, ty, name, EnemyManager::GetInstance()->getBoss(enemyId));
    enemy.life.isSpell = 1;
    _ins(538, spell2) _S(i) _S(t) _S(ty) _z(name) _args i -= 1;
    Spellcard::GetInstance()->Init(i + GLOBALS.inner.DIFFICULTY, t, ty, name, EnemyManager::GetInstance()->getBoss(enemyId));
    enemy.life.isSpell = 1;
    _ins(539, spell3) _S(i) _S(t) _S(ty) _z(name) _args i -= 2;
    Spellcard::GetInstance()->Init(i + GLOBALS.inner.DIFFICULTY, t, ty, name, EnemyManager::GetInstance()->getBoss(enemyId));
    enemy.life.isSpell = 1;
    ...
    _ins(544, unknown544) _S(a) _args enemy.flags = (enemy.flags ^ (enemy.flags ^ (a << 0x1b))) & 0x8000000;
    _alert;
    _ins(545, laserCancel) _args LASER_MANAGER_PTR->cancel_all(0);
    _ins(546, bombShield) _S(set) _S(scr) _args
        enemy.bombShieldOnAnmMain
        = scr;
    enemy.bombShieldOffAnmMain = enemy.anm0scr;
    enemy.flags &= 0xffffffffdffffffe;
    enemy.flags = (enemy.flags ^ (enemy.flags ^ (set << 0x1c))) & 0x10000000;
    _ins(547, gameSpeed) _f(s) _args _alert;
    _ins(548, diffWait) _S(e) _S(n) _S(h) _S(l) _args context.currentContext->time -= Diff(e, n, h, l);
    _ins(549, unknown549) _S(a) _args enemy.flags = 0x80000000 | (enemy.flags & 0xffffffff7fffffff);
    _alert;
    _ins(550, unknown550) _S(a) _args enemy.ecl550 = a;
    _alert;
    ...
    _ins(562, dropItemsSp) _args enemy.drops.eject_all_drops(enemy.final_pos.pos); // spawn item even in spell practice
    _ins(563, hbRect) _S(state) _args enemy.flags &= ~(1 << 12);
