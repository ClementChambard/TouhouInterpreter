...
int Enemy::execInstr(EclRunContext_t* cont, EclRawInstr_t* instr)
{
    ...
    _ins(405,      moveVelTime) _S(t) _S(m) _f(r) _f(s)                       _args
        if (t < 1) {
            ...
            if (m == 7) {
                if (r <= -999999.0) r = 0;
                else if ((enemy.flags & 0x80000U) != 0) r = -r;
                if (s <= -999999.0) s = 0;
            }
            else {
                if (r <= -999999.0) r = enemy.abs_pos.angle;
                else if ((enemy.flags & 0x80000U) != 0) r = PI - r;
                if (s <= -999999.0) s = enemy.abs_pos.speed;
            }

            enemy.abs_angle_i.start(enemy.abs_pos.angle, r, t, m); // do some wierd thing with angle, same each time play with angle interpolation
    ...
    _ins(408,         moveCirc) _f(t) _f(s) _f(r) _f(ri)                      _args
        if ((enemy.abs_pos.flags & 0xf) != 2) enemy.abs_pos.velocity = enemy.abs_pos.pos;
    ...
    _ins(409,     moveCircTime) _S(t) _S(m) _f(s) _f(r) _f(ri)                _args
        ...
        if (t < 1) {
            ...
            enemy.abs_speed_i.start(enemy.abs_pos.speed, s, t, m);
            enemy.abs_radial_dist_i.start({enemy.abs_pos.circle_radius, enemy.abs_pos.circle_radial_speed}, {r, ri}, t, m);
            enemy.abs_pos.flags = (enemy.abs_pos.flags & 0xfffffff0) | 2;
            enemy.abs_pos.update_position();
            enemy.calc_final_pos();
        }
    ...
    _ins(410,      moveCircRel) _f(t) _f(s) _f(r) _f(ri)                      _args
        if ((enemy.rel_pos.flags & 0xf) != 2) enemy.rel_pos.velocity = enemy.rel_pos.pos;
    ...
    _ins(411,  moveCircRelTime) _S(t) _S(m) _f(s) _f(r) _f(ri)                _args
        ...
        if (t < 1) {
            ...
            enemy.rel_speed_i.start(enemy.rel_pos.speed, s, t, m);
            enemy.rel_radial_dist_i.start({enemy.rel_pos.circle_radius, enemy.rel_pos.circle_radial_speed}, {r, ri}, t, m);
            enemy.rel_pos.flags = (enemy.rel_pos.flags & 0xfffffff0) | 2;
            enemy.rel_pos.update_position();
            enemy.calc_final_pos();
        }
    ...
    _ins(412,         moveRand) _S(t) _S(m) _f(s)                             _args
        float a;
        if (enemy.moveLimitPos.x - enemy.moveLimitSize.x * 0.25 <= enemy.final_pos.pos.x) {
        if (enemy.moveLimitSize.x * 0.25 + enemy.moveLimitPos.x < enemy.final_pos.pos.x) {
        ...
        else a = Random::Angle() / 3.f;
        if (enemy.moveLimitPos.y - enemy.moveLimitSize.y * 0.25 <= enemy.final_pos.pos.y) {
            if (enemy.moveLimitSize.y * 0.25 + enemy.moveLimitPos.y < enemy.final_pos.pos.y) a = -fabs(a);
        }
        else a = fabs(a);
        // wierd
        /*
        ...
        if (0.05 < fabs(a - PI1_2)) {
            if (PI1_2 <= a) a = 1.6207963;
            else a = 1.5207964;
        }
        }
        else if (-PI1_2 <= a) a = -1.5207964;
        else a = -1.6207963;*/
    ...
    _ins(413,      moveRandRel) _S(t) _S(m) _f(s)                             _args
        ...
        // wierd thing from above
        enemy.rel_angle_i.start(a, a, t, m);
        enemy.rel_speed_i.start(s, 0, t, m);
        enemy.rel_pos.flags &= 0xfffffff0;

    _ins(414,         moveBoss)                                               _args
        auto e = EnemyManager::GetInstance()->EnmFind(EnemyManager::GetInstance()->data.boss_ids[0]);
    ...
    _ins(421,      moveEllTime) _S(t) _S(m) _f(s) _f(r) _f(ri) _f(d) _f(q)    _args
        ...
        if (t < 1) {
            ...
            enemy.abs_speed_i.start(enemy.abs_pos.speed, s, t, m);
            enemy.abs_radial_dist_i.start({enemy.abs_pos.circle_radius, enemy.abs_pos.circle_radial_speed}, {r, ri}, t, m);
            enemy.abs_ellipse_i.start({enemy.abs_pos.some_angle1, enemy.abs_pos.some_value}, {q, d}, t, m);
    ...
    _ins(423,   moveEllRelTime) _S(t) _S(m) _f(s) _f(r) _f(ri) _f(d) _f(q)    _args
        ...
        if (t < 1) {
            ...
            enemy.rel_speed_i.start(enemy.rel_pos.speed, s, t, m);
            enemy.rel_radial_dist_i.start({enemy.rel_pos.circle_radius, enemy.rel_pos.circle_radial_speed}, {r, ri}, t, m);
            enemy.rel_ellipse_i.start({enemy.rel_pos.some_angle1, enemy.rel_pos.some_value}, {q, d}, t, m);
    ...
    _ins(425,       moveBezier) _S(t) _f(x) _f(y) _f(x2) _f(y2) _f(x3) _f(y3) _args
        ...
        enemy.abs_pos_i.start_bezier(enemy.abs_pos.pos, {x2, y2, 0.f}, {x, y, 0.f}, {x3, y3, 0.f}, t);
        enemy.abs_pos.flags &= 0xfffffff0;
    ...
    _ins(426,    moveBezierRel) _S(t) _f(x) _f(y) _f(x2) _f(y2) _f(x3) _f(y3) _args
        ...
        enemy.rel_pos_i.start_bezier(enemy.rel_pos.pos, {x2, y2, 0.f}, {x, y, 0.f}, {x3, y3, 0.f}, t);
        enemy.rel_pos.flags &= 0xfffffff0;
    ...
    _ins(434,        moveCurve) _S(t) _S(mx) _S(my) _f(x) _f(y)               _args
        if (t < 1) enemy.abs_pos_i.end_time = 0;
        else {
            if (x <= -999999.0) x = enemy.abs_pos.pos.x;
            if (y <= -999999.0) y = enemy.abs_pos.pos.y;
            enemy.abs_pos_i.start_curve(enemy.abs_pos.pos, {x, y, 0.f}, t, mx, my, 0);
            enemy.abs_pos.flags &= 0xfffffff0;
        }

    _ins(435,     moveCurveRel) _S(t) _S(mx) _S(my) _f(x) _f(y)               _args
        if (t < 1) enemy.rel_pos_i.end_time = 0;
        else {
            if (x <= -999999.0) x = enemy.rel_pos.pos.x;
            if (y <= -999999.0) y = enemy.rel_pos.pos.y;
            enemy.rel_pos_i.start_curve(enemy.rel_pos.pos, {x, y, 0.f}, t, mx, my, 0);
            enemy.rel_pos.flags &= 0xfffffff0;
        }
    ...
    _ins(436,      moveAddTime) _S(t) _S(m) _f(x) _f(y)                       _args
        if (t < 1) {
            ...
            if ((enemy.flags & 0x80000U) == 0) x += enemy.abs_pos.pos.x;
            else x = enemy.abs_pos.pos.x - x;
    ...
    _ins(437,   moveAddRelTime) _S(t) _S(m) _f(x) _f(y)                       _args
        if (t < 1) {
            ...
            if ((enemy.flags & 0x80000U) == 0) x += enemy.rel_pos.pos.x;
            else x = enemy.rel_pos.pos.x - x;
    ...
    _ins(441,    moveAngleTime) _S(t) _S(m) _f(a)                             _args
        if (0 < t) {
            if (m == 7) {
                if (a <= -999999.0) a = 0.f;
                else if ((enemy.flags & 0x80000U) != 0) a = -a;
            }
            else if (a <= -999999.0) a = enemy.abs_pos.angle;
            else if ((enemy.flags & 0x80000U) != 0) a = PI - a;
            enemy.abs_angle_i.start(enemy.abs_pos.angle, a, t, m);
            enemy.abs_pos.flags &= 0xfffffff0;
        }
        else enemy.abs_angle_i.end_time = 0;
    ...
    _ins(443, moveAngleRelTime) _S(t) _S(m) _f(a)                             _args
        if (0 < t) {
            ...
            else if (a <= -999999.0) a = enemy.rel_pos.angle;
            else if ((enemy.flags & 0x80000U) != 0) a = PI - a;
            enemy.rel_angle_i.start(enemy.rel_pos.angle, a, t, m);
            enemy.rel_pos.flags &= 0xfffffff0;
        }
        else enemy.rel_angle_i.end_time = 0;
    ...
    _ins(445,    moveSpeedTime) _S(t) _S(m) _f(s)                             _args
        ...
        else enemy.abs_speed_i.end_time = 0;
    ...
    _ins(447, moveSpeedRelTime) _S(t) _S(m) _f(s)                             _args
        ...
        else enemy.rel_speed_i.end_time = 0;
