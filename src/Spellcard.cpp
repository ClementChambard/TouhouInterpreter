...
void Spellcard::update() {
    ...
    if (!vm) {
        ...
        auto e = EnemyManager::GetInstance()->EnmFind(
            EnemyManager::GetInstance()->getData()->boss_ids[bossId]);
        if (e) {
            spell_circle_x = (e->getData()->final_pos.pos.x - spell_circle_x)
                * 0.05 + spell_circle_x;
            spell_circle_y = (e->getData()->final_pos.pos.y - spell_circle_y)
                * 0.05 + spell_circle_y;
            spell_circle_z = (e->getData()->final_pos.pos.z - spell_circle_z)
                * 0.05 + spell_circle_z;
...
void Spellcard::Init(int /*id*/, int time, int /*mode*/,
                     std::string /*name*/, int bossId) {
    ...
    spell_bg_anm_id = AnmManager::SpawnVM(
        STAGE_DATA_TABLE[GLOBALS.inner.STAGE_NUM]["boss_data"][bossId]
        ["spell_bg_anim_index"].asInt(),
        STAGE_DATA_TABLE[GLOBALS.inner.STAGE_NUM]["boss_data"][bossId]
        ["spell_bg_anm_script"].asInt());
    AnmManager::SpawnVM(STAGE_DATA_TABLE[GLOBALS.inner.STAGE_NUM]["boss_data"]
                        [bossId]["spell_portrait_anim_index"].asInt(),
                        STAGE_DATA_TABLE[GLOBALS.inner.STAGE_NUM]["boss_data"]
                        [bossId]["spell_portrait_anm_script"].asInt());
}
