...
struct File {
    ...
    ns::Texture* getTextureFromName(cstr name) const;
    ...
    ID spawnVMExt(i32 id, f32 rot, ns::vec3 const& pos, i32 mode,
    ...
    ID createEffectFront(i32 id, i32 layer = -1, VM** output
                         = nullptr);
    ID createEffectPos(i32 id, f32 rot, ns::vec3 const& pos,
                       i32 layer = -1, VM** out_vm = nullptr);
