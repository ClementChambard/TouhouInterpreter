...
void setup_render_state_for_vm_default_tex(VM *vm);
...
struct AnmManagerState {
    ...
    ns::Texture *last_used_texture = nullptr;
...
static ns::Texture *default_texture = nullptr;
...
inline void use_vm_texture(VM *vm) {
    anm::use_texture(vm->getSprite().texture);
}

void use_texture(ns::Texture *tex) {
    if (AM->last_used_texture != tex) {
        flush_vbos();
        AM->last_used_texture = tex;
        ...
        AM->last_used_texture->use(0);
    }
}

void use_default_texture() {
    if (AM->last_used_texture != default_texture) {
        flush_vbos();
        AM->last_used_texture = default_texture;
        AM->last_used_texture->use(0);
    }
}

void init() {
    default_texture = ns::get_default_texture();
...
void delete_vm(VM *vm) {
    if (vm == nullptr || vm->bitflags.f534_26) {
        return;
    }
...
i32 on_tick_world() {
    ...
    for (auto node = AM->world_list_head; node; node = node->next) {
        ...
        if (node->value->bitflags.activeFlags == 0b01 || (node->value->bitflags.activeFlags == 0b00 && node->value->update())) {
...
static void anm_use_resamplemode(u32 resamplemode) {
    if (AM->last_used_resamplemode != resamplemode) {
        flush_vbos();
        if (resamplemode == 0) {
            AM->last_used_texture->set_min_mag_filter(ns::Texture::Filter::LINEAR);
        } else {
            AM->last_used_texture->set_min_mag_filter(ns::Texture::Filter::NEAREST);
        }
...
static void anm_use_scrollmode_x(u32 scrollmode) {
    if (AM->last_used_scrollmodex != scrollmode) {
        ...
        if (AM->last_used_scrollmodex == 0) {
            AM->last_used_texture->set_wrapping_u(ns::Texture::Wrapping::REPEAT);
        } else if (AM->last_used_scrollmodex == 1) {
            AM->last_used_texture->set_wrapping_u(ns::Texture::Wrapping::CLAMP);
        } else if (AM->last_used_scrollmodex == 2) {
            AM->last_used_texture->set_wrapping_u(ns::Texture::Wrapping::MIRROR);
        }
    }
}
...
static void anm_use_scrollmode_y(u32 scrollmode) {
    if (AM->last_used_scrollmodey != scrollmode) {
        ...
        if (AM->last_used_scrollmodey == 0) {
            AM->last_used_texture->set_wrapping_v(ns::Texture::Wrapping::REPEAT);
        } else if (AM->last_used_scrollmodey == 1) {
            AM->last_used_texture->set_wrapping_v(ns::Texture::Wrapping::CLAMP);
        } else if (AM->last_used_scrollmodey == 2) {
            AM->last_used_texture->set_wrapping_v(ns::Texture::Wrapping::MIRROR);
        }
    }
}
...
void draw_vm(VM *vm) {
    ...
    //     f32 d = ns::point_distance({vm->pos + vm->__pos_2 + vm->entity_pos}, SUPERVISOR.current_camera->position);
    //     f32 factor = (d - vm->cam_fade_begin) / (vm->cam_fade_end-vm->cam_fade_begin);
    //     factor = std::clamp(factor, 0.f, 1.f);
    //     vm->color_1.a *= factor;
    //     vm->color_2.a *= factor;
    // }

    if (vm->bitflags.rendermode != 8 && vm->bitflags.rendermode != 15 && vm->bitflags.rendermode != 24 && vm->bitflags.rendermode != 25) {
        disable_zwrite();
    }
    ...
    case 7: {
        ...
        calc_mat_world(vm);
        auto col = vm->bitflags.colmode ? vm->color_2 : vm->color_1;
        for (i32 i = 0; i < 4; i++) {
            ns::vec3 v = ns::vec3(AM->__matrix_186017c * ns::vec4(AM->some_positions[2], AM->some_positions[3], AM->some_positions[4], 1));
            if (ns::point_distance(v, AM->current_camera->position) <= AM->current_camera->sky.begin_distance) {
                SPRITE_TEMP_BUFFER[i].diffuse_color = col;
            } else {
                f32 coeff = (AM->current_camera->sky.begin_distance - ns::point_distance(v, AM->current_camera->position)) /
                            (AM->current_camera->sky.begin_distance - AM->current_camera->sky.end_distance);
                if (coeff < 1.0) {
                    SPRITE_TEMP_BUFFER[i].diffuse_color.b = col.b - ((col.b - AM->current_camera->sky.color_components[0]) * coeff);
                    SPRITE_TEMP_BUFFER[i].diffuse_color.g = col.g - ((col.g - AM->current_camera->sky.color_components[1]) * coeff);
                    SPRITE_TEMP_BUFFER[i].diffuse_color.r = col.r - ((col.r - AM->current_camera->sky.color_components[2]) * coeff);
    ...
    case 0xd:
    case 0xe:
        draw_vm_as_triangle_strip(vm, reinterpret_cast<RenderVertex_t *>(vm->special_vertex_buffer_data), vm->int_script_vars[0] * 2);
        break;
    case 0xb:
        draw_vm_triangle_fan(vm, reinterpret_cast<RenderVertex_t *>(vm->special_vertex_buffer_data), vm->int_script_vars[0] * 2);
        break;
    case 0xf:
        if (vm->color_1.a == 0 && vm->color_2.a == 0) {
            break;
    ...
    case 0x1b: {
        ...
        setup_render_state_for_vm_default_tex(vm);
        ...
        switch (vm->bitflags.rendermode) {
        case 0x10:
            draw_vm__ins_603(pos.x, pos.y, xs, ys, rot, vm->color_1, vm->color_1, vm->bitflags.anchorX, vm->bitflags.anchorY);
            break;
        case 0x14:
            draw_vm__ins_603(pos.x, pos.y, xs, ys, rot, vm->color_1, vm->color_2, vm->bitflags.anchorX, vm->bitflags.anchorY);
            break;
        case 0x15:
            draw_vm__ins_607(pos.x, pos.y, xs, ys, rot, vm->color_1, vm->color_1, vm->bitflags.anchorX, vm->bitflags.anchorY);
            break;
        case 0x16:
            draw_vm__ins_607(pos.x, pos.y, xs, ys, rot, vm->color_1, vm->color_2, vm->bitflags.anchorX, vm->bitflags.anchorY);
            break;
        case 0x1a:
            draw_vm__ins_613(pos.x, pos.y, xs, rot, vm->color_1, (vm->bitflags.colmode == 0) ? vm->color_1 : vm->color_2, vm->bitflags.anchorX);
            break;
        case 0x1b:
            draw_vm__ins_612(pos.x, pos.y, xs, ys, rot, vm->color_1, (vm->bitflags.colmode == 0) ? vm->color_1 : vm->color_2, vm->bitflags.anchorX, vm->bitflags.anchorY);
            break;
        }
        break;
    }
    ...
    case 0x13: {
        ...
        if (vm->bitflags.rendermode == 0x11) {
            ...
            draw_vm__mode_19__drawRing(pos.x, pos.y, rot, xs, ys, vm->int_script_vars[0], vm->color_1);
        }
    } break;
    case 0x18:
    case 0x19:
        draw_vm_mode_24_25(vm, vm->special_vertex_buffer_data, vm->int_script_vars[0] * 2);
        break;
    }
...
void draw_vm__modes_0_1_2_3(VM *vm, i32 i) {
    ...
    if (!(i & 1)) {
        SPRITE_TEMP_BUFFER[0].transformed_pos.x = std::round(SPRITE_TEMP_BUFFER[0].transformed_pos.x) - 0.5;
        SPRITE_TEMP_BUFFER[0].transformed_pos.y = std::round(SPRITE_TEMP_BUFFER[0].transformed_pos.y) - 0.5;
        SPRITE_TEMP_BUFFER[1].transformed_pos.x = std::round(SPRITE_TEMP_BUFFER[1].transformed_pos.x) - 0.5;
        SPRITE_TEMP_BUFFER[1].transformed_pos.y = std::round(SPRITE_TEMP_BUFFER[1].transformed_pos.y) - 0.5;
        SPRITE_TEMP_BUFFER[2].transformed_pos.x = std::round(SPRITE_TEMP_BUFFER[2].transformed_pos.x) - 0.5;
        SPRITE_TEMP_BUFFER[2].transformed_pos.y = std::round(SPRITE_TEMP_BUFFER[2].transformed_pos.y) - 0.5;
        SPRITE_TEMP_BUFFER[3].transformed_pos.x = std::round(SPRITE_TEMP_BUFFER[3].transformed_pos.x) - 0.5;
        SPRITE_TEMP_BUFFER[3].transformed_pos.y = std::round(SPRITE_TEMP_BUFFER[3].transformed_pos.y) - 0.5;
    }
    ...
    if (!(i & 4)) {
        ...
        if (fmax(fmax(SPRITE_TEMP_BUFFER[0].transformed_pos.x, SPRITE_TEMP_BUFFER[1].transformed_pos.x),
                 fmax(SPRITE_TEMP_BUFFER[2].transformed_pos.x, SPRITE_TEMP_BUFFER[3].transformed_pos.x)) <
                 AM->current_camera->viewport.X) {
            return;
        }

        /* if max of y pos is smaller than top of viewport --> discard drawing */
        if (fmax(fmax(SPRITE_TEMP_BUFFER[0].transformed_pos.y, SPRITE_TEMP_BUFFER[1].transformed_pos.y),
                 fmax(SPRITE_TEMP_BUFFER[2].transformed_pos.y, SPRITE_TEMP_BUFFER[3].transformed_pos.y)) <
                 AM->current_camera->viewport.Y) {
            return;
        }

        /* if min of x pos is larger than right of viewport --> discard drawing */
        if (fmin(fmin(SPRITE_TEMP_BUFFER[0].transformed_pos.x, SPRITE_TEMP_BUFFER[1].transformed_pos.x),
                 fmin(SPRITE_TEMP_BUFFER[2].transformed_pos.x, SPRITE_TEMP_BUFFER[3].transformed_pos.x)) >
        ...
        if (fmin(fmin(SPRITE_TEMP_BUFFER[0].transformed_pos.y, SPRITE_TEMP_BUFFER[1].transformed_pos.y),
                 fmin(SPRITE_TEMP_BUFFER[2].transformed_pos.y, SPRITE_TEMP_BUFFER[3].transformed_pos.y)) >
    ...
    ns::Vertex tl = {ns::vec3(SPRITE_TEMP_BUFFER[0].transformed_pos), SPRITE_TEMP_BUFFER[0].diffuse_color, SPRITE_TEMP_BUFFER[0].texture_uv};
    ns::Vertex tr = {ns::vec3(SPRITE_TEMP_BUFFER[2].transformed_pos), SPRITE_TEMP_BUFFER[2].diffuse_color, SPRITE_TEMP_BUFFER[2].texture_uv};
    ns::Vertex br = {ns::vec3(SPRITE_TEMP_BUFFER[3].transformed_pos), SPRITE_TEMP_BUFFER[3].diffuse_color, SPRITE_TEMP_BUFFER[3].texture_uv};
    ns::Vertex bl = {ns::vec3(SPRITE_TEMP_BUFFER[1].transformed_pos), SPRITE_TEMP_BUFFER[1].diffuse_color, SPRITE_TEMP_BUFFER[1].texture_uv};
...
i32 draw_vm__mode_6(VM *vm) {
    ...
    ns::vec3 pos_from_cam = vm->entity_pos + vm->pos + vm->__pos_2 - AM->current_camera->position;
    if (vm->bitflags.originMode != 0 && !vm->parent_vm) {
        pos_from_cam += ns::vec3(BACK_BUFFER_SIZE.x * 0.5, (BACK_BUFFER_SIZE.y - 448.0) * 0.5, 0);
    }
    ...
    default:
        ...
        fog_factor = (dist_to_cam - AM->current_camera->sky.begin_distance) / (AM->current_camera->sky.end_distance - AM->current_camera->sky.begin_distance);
        if (fog_factor > 1.0) {
            return -1;
        }
        SPRITE_TEMP_BUFFER[0].diffuse_color.r = col.r - (col.r - AM->current_camera->sky.color_components[2]) * fog_factor;
        SPRITE_TEMP_BUFFER[0].diffuse_color.g = col.g - (col.g - AM->current_camera->sky.color_components[1]) * fog_factor;
        SPRITE_TEMP_BUFFER[0].diffuse_color.b = col.b - (col.b - AM->current_camera->sky.color_components[0]) * fog_factor;
    ...
    case 3:
        ...
        if (AM->current_camera->sky.begin_distance < dist_to_cam) {
            fog_factor = (AM->current_camera->sky.begin_distance - dist_to_cam) / (AM->current_camera->sky.begin_distance - AM->current_camera->sky.end_distance);
            if (1.0 <= fog_factor) {
                return -1;
            }
            SPRITE_TEMP_BUFFER[0].diffuse_color.r = col.r - (col.r - AM->current_camera->sky.color_components[2]) * fog_factor;
            ...
            SPRITE_TEMP_BUFFER[3].diffuse_color.r = col2.r - (col2.r - AM->current_camera->sky.color_components[2]) * fog_factor;
            SPRITE_TEMP_BUFFER[3].diffuse_color.g = col2.g - (col2.g - AM->current_camera->sky.color_components[1]) * fog_factor;
            SPRITE_TEMP_BUFFER[3].diffuse_color.b = col2.b - (col2.b - AM->current_camera->sky.color_components[0]) * fog_factor;
...
void draw_vm__mode_8_15(VM *vm) {
    ...
    local_90[3][0] = vm->entity_pos.x + vm->pos.x + vm->__pos_2.x - vm->anchor_offset.x * vm->scale.x * vm->scale_2.x;
    local_90[3][1] = vm->entity_pos.y + vm->pos.y + vm->__pos_2.y - vm->anchor_offset.y * vm->scale.y * vm->scale_2.y;
...
void draw_vm_triangle_fan(VM *vm, RenderVertex_t *vertexData, i32 nVert) {
    ...
    // }
    // draw data in buffer
    // SUPERVISOR.d3d_device->DrawPrimitiveUP(D3DPT_TRIANGLEFAN, nVert - 2, vertexData, 0x1c);

    // My version
    u32 center = AM->batch->add_vertex({ns::vec3(vertexData[0].transformed_pos), vertexData[0].diffuse_color, vertexData[0].texture_uv});
    u32 last = AM->batch->add_vertex({ns::vec3(vertexData[1].transformed_pos), vertexData[1].diffuse_color, vertexData[1].texture_uv});
    for (i32 i = 2; i < nVert; i++) {
        u32 new_vert = AM->batch->add_vertex({ns::vec3(vertexData[i].transformed_pos), vertexData[i].diffuse_color, vertexData[i].texture_uv});
...
void draw_vm_as_triangle_strip(VM *vm, RenderVertex_t *vertexData, usize nVert) {
    ...
    use_vm_texture(vm);
    setup_render_state_for_vm(vm);
    u32 last2 = AM->batch->add_vertex({ns::vec3(vertexData[0].transformed_pos), vertexData[0].diffuse_color, vertexData[0].texture_uv});
    u32 last = AM->batch->add_vertex({ns::vec3(vertexData[1].transformed_pos), vertexData[1].diffuse_color, vertexData[1].texture_uv});
    for (usize i = 2; i < nVert; i++) {
        u32 new_vert = AM->batch->add_vertex({ns::vec3(vertexData[i].transformed_pos), vertexData[i].diffuse_color, vertexData[i].texture_uv});
        AM->batch->draw_tri(last2, i % 2 ? new_vert : last, i % 2 ? last : new_vert);
        last2 = last;
        last = new_vert;
    }
    // if (field_0x18607ca != 3) {
    ...
    //     field_0x18607cf = 1;
    // }
    // SUPERVISOR.d3d_device->DrawPrimitiveUP(D3DPT_TRIANGLESTRIP, nVert - 2, vertexData, 0x1c);
}

i32 draw_vm__ins_607(f32 x, f32 y, f32 width, f32 height, f32 rot_z, ns::Color col1, ns::Color col2, i32 anchorX, i32 anchorY) {
    draw_vm__ins_603(x, y, width + 1.0, height + 1.0, rot_z, {col1.r, col1.g, col1.b, static_cast<u8>(col1.a / 2)}, {col2.r, col2.g, col2.b, static_cast<u8>(col2.a / 2)}, anchorX, anchorY);
...
i32 draw_vm__ins_603(f32 x, f32 y, f32 width, f32 height, f32 rot_z, ns::Color col1, ns::Color col2, i32 anchorX, i32 anchorY) {
    ...
    disable_zwrite();
    anm::use_default_texture();
    AM->batch->draw_quad({{pos0, 0.f}, col1, {}}, {{pos1, 0.f}, col2, {}}, {{pos2, 0.f}, col2, {}}, {{pos3, 0.f}, col1, {}});
    return 0;
    // flush_vbos();
    // some_d3d_stuff();
...
i32 draw_vm__ins_613(f32 x, f32 y, f32 width, f32 rot_z, ns::Color col1, ns::Color col2, i32 anchor) {
    ...
    AM->batch->draw_quad({{pos0 - dir, 0.f}, col1, {}}, {{pos0 + dir, 0.f}, col1, {}}, {{pos1 + dir, 0.f}, col2, {}}, {{pos1 - dir, 0.f}, col2, {}});
    some_d3d_stuff();
...
i32 draw_vm__ins_612(f32 x, f32 y, f32 width, f32 height, f32 rotz, ns::Color col1, ns::Color col2, i32 anchorX, i32 anchorY) {
    f32 x1 = anchorX == 0 ? -width / 2.0 : anchorX == 1 ? 0.0 : -width;
    f32 x2 = anchorX == 0 ? width / 2.0 : anchorX == 1 ? width : 0.0;
    f32 y1 = anchorY == 0 ? -height / 2.0 : anchorY == 1 ? 0.0 : -height;
    f32 y2 = anchorY == 0 ? height / 2.0 : anchorY == 1 ? height : 0.0;
...
void set_effect_508(usize i, int16_t anm_loaded_index, int16_t script_index, i32 index_of_on_create, i32 index_of_on_tick,
                    i32 index_of_on_draw, i32 index_of_on_destroy, i32 index_of_on_interrupt, i32 index_of_on_copy_1__disused,
...
ID createVM508(i32 i, VM *root) {
    if (AM->effect_table.size() <= static_cast<usize>(i) || AM->effect_table[i].anm_loaded_index < 0) {
        return 0;
    }
    if (-1 < AM->effect_table[i].script_index) {
        auto vm = root;
        if (!vm) {
            AM->loadedFiles[AM->effect_table[i].anm_loaded_index].createEffect(AM->effect_table[i].script_index, -1, &vm);
        }
        if (AM->effect_table[i].index_of_on_create && Funcs::on_create(AM->effect_table[i].index_of_on_create)) {
        ...
        vm->index_of_on_copy_1__disused = AM->effect_table[i].index_of_on_copy_1__disused;
        vm->index_of_on_copy_2__disused = AM->effect_table[i].index_of_on_copy_2__disused;
        return vm->id;
    }
    return 0;
}
...
void set_fog_params(ns::Color col, f32 beg, f32 end) {
    ...
    AM->fshader->use();
    AM->fshader->SetFog(beg, end, {col.r / 255.f, col.g / 255.f, col.b / 255.f, col.a / 255.f});
}
...
void draw_rect_col(ns::vec4 const &rect, ns::Color c) {
    ...
    AM->batch->draw_quad({{rect.x, rect.y, 0.f}, c, {0.f, 0.f}}, {{rect.z, rect.y, 0.f}, c, {1.f, 0.f}}, {{rect.z, rect.w, 0.f}, c, {1.f, 1.f}}, {{rect.x, rect.w, 0.f}, c, {0.f, 1.f}});
    flush_vbos();
...
void draw_rect_col(ns::vec4 const &rect, i32 a, i32 rgb) {
    ns::Color c = {static_cast<u8>((rgb >> 0x10) & 0xff), static_cast<u8>((rgb >> 0x08) & 0xff), static_cast<u8>((rgb >> 0x00) & 0xff), static_cast<u8>(a & 0xff)};
...
ns::vec2 &origin(usize i) {
    return AM->origins[i];
}

Camera_t *get_3d_camera() {
    return AM->_3d_camera;
}
...
void raw_batch_draw(u32 texture, RenderVertex_t tl, RenderVertex_t tr, RenderVertex_t br, RenderVertex_t bl, u8 blendmode) {
    NS_ERROR("USE OF DEPRECATED FUNCTION");
    (void)texture, (void)tl, (void)tr, (void)br, (void)bl, (void)blendmode;
}

void raw_batch_draw(ns::Texture *texture, RenderVertex_t tl, RenderVertex_t tr, RenderVertex_t br, RenderVertex_t bl, u8 blendmode) {
    anm::use_texture(texture);
