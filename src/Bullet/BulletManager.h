...
public:
    ...
    void cancel_radius(ns::vec3 const& pos, int item, float r);
    void cancel_radius_as_bomb(ns::vec3 const& pos, int item, float r);
    void cancel_rectangle_as_bomb(float angle, ns::vec2 const& pos, ns::vec2 const& scale, int item);
...
// private:
    ...
    void ShootSingle(EnemyBulletShooter_t* bh, float a, float s, ns::vec2 pos);
    int on_tick();
    int on_draw();
