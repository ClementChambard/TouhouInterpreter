...
int BULLET_ADDITIONAL_CANCEL_SCR[18] = {0x4,  0x8,  0xC, 0x10, 0x14, 0x18,
                                        0x1C, 0x22, 0x0, 0x1,  0x2,  0x3,
                                        -1,   0x4,  0x5, 0x6,  0x7,  -1};

BulletManager *BULLET_MANAGER_PTR = nullptr;

static int32_t on_sprite_set_bullet(anm::VM *vm, int32_t v) {
  auto b = reinterpret_cast<Bullet *>(vm->associated_game_entity);
...
void BulletManager::ShootSingle(EnemyBulletShooter_t *bh, float a, float s,
                                ns::vec2 pos) {
  ...
  b->pos = ns::vec3(
      pos + ns::vec2{bh->distance * ns::cos(a), bh->distance * ns::sin(a)}, 0);
  ...
  b->__timer_e54 = 0;
  b->__timer_e68 = 0;
  b->velocity = {s * ns::cos(a), s * ns::sin(a), 0.f};
  b->color = bh->__color;
  b->type = bh->type;
  // b->field21_0x680 = 0;
  // b->field32_0xe4c = 60;
  ...
  case 6:
    b->cancel_sprite_id =
        b->sprite_data["colors"][bh->__color]["cancel_script"].asInt();
    break;
  case 7:
  ...
  if (bh->ex[bh->__start_transform].type == 2) {
    ...
    b->state = 2;
    b->pos -= ns::vec3(b->speed * ns::cos(b->angle),
                       b->speed * ns::sin(b->angle), 0.f) *
              4.f;
...
void BulletManager::cancel_radius(ns::vec3 const &pos, int item, float r) {
...
void BulletManager::cancel_radius_as_bomb(ns::vec3 const &pos, int item,
                                          float r) {
  auto node = tick_list_head.next;
  auto next_node = node;
  Bullet *bullet = nullptr;
...
void BulletManager::cancel_rectangle_as_bomb(float angle, ns::vec2 const &pos,
                                             ns::vec2 const &size, int item) {
  for (int i = 0; i < 2000; i++) {
    ...
    float xx = ns::abs(ns::cos(-angle) * (bullet->pos.x - pos.x) -
                       ns::sin(-angle) * (bullet->pos.y - pos.y));
    float yy = ns::abs(ns::cos(-angle) * (bullet->pos.y - pos.y) +
                       ns::sin(-angle) * (bullet->pos.x - pos.x));
    if (size.x * 0.5 + r < xx || (size.y * 0.5 < yy && size.x * 0.5 < xx) ||
        size.y * 0.5 + r < yy) {
