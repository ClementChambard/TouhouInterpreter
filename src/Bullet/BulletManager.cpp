...
void BulletManager::ShootSingle(EnemyBulletShooter_t* bh,
    ...
    b->pos = ns::vec3(pos +
                ns::vec2 { bh->distance * ns::cos(a), bh->distance * ns::sin(a) }, 0);
    ...
    if (bh->ex[bh->__start_transform].type == 2) {
        ...
        b->pos -= ns::vec3(b->speed * ns::cos(b->angle),
                            b->speed * ns::sin(b->angle), 0.f) * 4.f;
    ...
    while (bullet) {
        if ((bullet->state == 2 || bullet->state == 1) &&
            math::point_distance_sq(bullet->pos, pos) <=
                  (bullet->hitbox_diameter / 2.f + r) * (bullet->hitbox_diameter / 2.f + r)) {
            bullet->cancel(item);
        }
        node = iter_next;
        if (node) {
...
void BulletManager::cancel_radius_as_bomb(ns::vec3 const& pos, int item, float r) {
    auto node = tick_list_head.next;
    auto next_node = node;
    Bullet* bullet = nullptr;
    if (node) {
...
void BulletManager::cancel_rectangle_as_bomb(float angle, ns::vec2 const& pos, ns::vec2 const& size, int item) {
