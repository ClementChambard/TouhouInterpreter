...
std::string int_print_n(int i, int n, char c = ' ') {
    ...
    while (s.length() < static_cast<size_t>(n)) s = c + s;
    return s;
}
...
void print_ecl(EclRaw_t* ecl) {
    ...
    for (auto sp : ecl->subs) {
        ...
        for (auto instr : sp.instrs) {
            std::cout << int_print_n(instr->time, 3) << "-" <<
                int_print_n(instr->size, 3, '-') << "-" <<
                rank_str(instr->rank_mask) << ": ";
            ...
            if (instr->param_count > 0) {
                ...
                thecl_value_t* values = thecl_value_list_from_data(instr->data,
                        instr->size - sizeof(EclRawInstr_t), format.c_str());
