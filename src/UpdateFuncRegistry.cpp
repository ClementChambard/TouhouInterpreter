...
int UpdateFuncRegistry::run_all_on_tick()
{
    ...
    UpdateFuncList_t *current_node = on_game_tick_list_head.list_node.next;
    int ret = 0;
    do {
        do {
            /* check the next non-empty func */
            if (!current_node) return ret;
            this->current_node = current_node->next;
            current_update_func = current_node->entry;
            ...
            if (!is_cleaning_up) {
                ...
                switch(current_update_func->function_ptr()) {
                    ...
                    default:
                        ...
                        break;
                    case 2:
                        if (current_update_func->flags & 2) goto Passtest;
                        current_node = this->current_node;
                        ret++;
                        break;
                    case 3:
                    ...
                    case 5:
                        return -1;
                    case 6:
                        current_node = on_game_tick_list_head.list_node.next;
                        ret = 0;
                        break;
                    case 7:
                        if (current_update_func->on_cleanup) current_update_func->on_cleanup();
                        current_node = this->current_node;
                        ret++;
                        break;
                }
...
int UpdateFuncRegistry::run_all_on_draw()
{
    UpdateFunc *func;
    int ret = 0;
    UpdateFuncList_t* current_node = on_draw_list_head.list_node.next;
    do {
        do {
            /* check the next non-empty func */
            if (!current_node) return ret;
            this->current_node = current_node->next;
            func = current_node->entry;
            current_node = current_node->next;
        ...
        PassTestdraw:
            switch(func->function_ptr()) {
                ...
                default:
                    break;
                case 2:
                    if (func->flags & 2) goto PassTestdraw;
                    break;
                case 3:
                    return 1;
                case 4:
...
int UpdateFuncRegistry::register_on_tick(UpdateFunc *func, int32_t priority)
{
    ...
    UpdateFuncList_t* current_node = on_game_tick_list_head.list_node.next;
    while ((current_node != nullptr && (current_node->entry->priority < priority))) {
...
{
    if (!func) return;
    if (func->list_node.prev) func->list_node.prev->next = func->list_node.next;
    if (func->list_node.next) func->list_node.next->prev = func->list_node.prev;
    delete func;
}
...
void UpdateFuncRegistry::unregister_from(UpdateFuncList_t *node)
{
    ...
    while (node != nullptr) {
        ...
        if (func != nullptr) unregister(func);
    }
}
