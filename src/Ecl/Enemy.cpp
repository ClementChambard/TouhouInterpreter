...
int EnemyData::step_interpolators() {
    ...
    if (vm0) {
        ...
        finalSpriteSize.y = ns::abs(s.h * vm0->scale.y);
    } else {
        anmIds[0].val = 0;
    }
    ...
    if (enemy.slowdown > 0) {
        ...
        if (GAME_SPEED - enemy.slowdown * GAME_SPEED >= 0) {
            if (GAME_SPEED - enemy.slowdown * GAME_SPEED <= 1.0)
                ns::get_instance()->set_game_speed(GAME_SPEED - enemy.slowdown * GAME_SPEED);
            else
                ns::get_instance()->set_game_speed(1.f);
        } else {
            ns::get_instance()->set_game_speed(0.f);
        }
        ...
        ns::get_instance()->set_game_speed(saved_game_speed);
        return ret;
    }
