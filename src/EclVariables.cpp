...
int32_t Enemy::checkVarI(int32_t id)
{
    ...
    if (id == -9999) return Random::Float01();
    if (id == -9998) return Random::Angle();
    if (id == -9997) return enemy.final_pos.pos.x;
    if (id == -9996) return enemy.final_pos.pos.y;
    if (id == -9995) return enemy.abs_pos.pos.x;
    if (id == -9994) return enemy.abs_pos.pos.y;
    if (id == -9993) return enemy.rel_pos.pos.x;
    if (id == -9992) return enemy.rel_pos.pos.y;
    if (id == -9991) return Globals::get()->playerX;
    if (id == -9990) return Globals::get()->playerY;
    if (id == -9989) return math::point_direction(enemy.final_pos.pos.x, enemy.final_pos.pos.y, Globals::get()->playerX, Globals::get()->playerY);
    if (id == -9988) return context.currentContext->time;
    if (id == -9987) return Random::Floatm11();
    ...
    if (id == -9981) return enemy.ecl_float_vars[0];
    if (id == -9980) return enemy.ecl_float_vars[1];
    if (id == -9979) return enemy.ecl_float_vars[2];
    if (id == -9978) return enemy.ecl_float_vars[3];
    if (id == -9977) return enemy.final_pos.pos.x;
    if (id == -9976) return enemy.final_pos.pos.y;
    if (id == -9975) return enemy.abs_pos.pos.x;
    if (id == -9974) return enemy.abs_pos.pos.y;
    if (id == -9973) return enemy.rel_pos.pos.x;
    if (id == -9972) return enemy.rel_pos.pos.y;
    if (id == -9971) return enemy.abs_pos.angle;
    if (id == -9970) return enemy.rel_pos.angle;
    if (id == -9969) return enemy.abs_pos.speed;
    if (id == -9968) return enemy.rel_pos.speed;
    if (id == -9967) return enemy.abs_pos.circle_radius;
    if (id == -9966) return enemy.rel_pos.circle_radius;
    if (id == -9965) return Globals::get()->playerX;
    if (id == -9964) return Globals::get()->playerY;
    if (id == -9963) return EnemyManager::GetInstance()->EnmFind(EnemyManager::GetInstance()->data.boss_ids[0])->enemy.final_pos.pos.x;
    if (id == -9962) return EnemyManager::GetInstance()->EnmFind(EnemyManager::GetInstance()->data.boss_ids[0])->enemy.final_pos.pos.y;
    if (id == -9961) return enemy.anmIds[0].val;
    if (id == -9960) return Globals::get()->rank;
    if (id == -9959) return Globals::get()->difficulty;
    if (id == -9958) return enemy.final_pos.angle;
    if (id == -9957) return true;
    if (id == -9956) return math::point_direction(enemy.abs_pos.pos.x, enemy.abs_pos.pos.y, Globals::get()->playerX, Globals::get()->playerY);
    if (id == -9955) return math::point_direction(enemy.rel_pos.pos.x, enemy.rel_pos.pos.y, Globals::get()->playerX, Globals::get()->playerY);
    ...
    if (id == -9944) return math::point_distance(enemy.final_pos.pos.x, enemy.final_pos.pos.y, Globals::get()->playerX, Globals::get()->playerY);
    ...
    if (id == -9939) return EnemyManager::GetInstance()->EnmFind(EnemyManager::GetInstance()->data.boss_ids[0])->enemy.ecl_float_vars[0];
    if (id == -9938) return EnemyManager::GetInstance()->EnmFind(EnemyManager::GetInstance()->data.boss_ids[0])->enemy.ecl_float_vars[1];
    if (id == -9937) return EnemyManager::GetInstance()->EnmFind(EnemyManager::GetInstance()->data.boss_ids[0])->enemy.ecl_float_vars[2];
    if (id == -9936) return EnemyManager::GetInstance()->EnmFind(EnemyManager::GetInstance()->data.boss_ids[0])->enemy.ecl_float_vars[3];
    if (id == -9935) return enemy.ecl_float_vars[4];
    if (id == -9934) return enemy.ecl_float_vars[5];
    if (id == -9933) return enemy.ecl_float_vars[6];
    if (id == -9932) return enemy.ecl_float_vars[7];
    ...
    if (id == -9922) return EnemyManager::GetInstance()->data.ecl_float_vars[0];
    if (id == -9921) return EnemyManager::GetInstance()->data.ecl_float_vars[1];
    if (id == -9920) return EnemyManager::GetInstance()->data.ecl_float_vars[2];
    if (id == -9919) return EnemyManager::GetInstance()->data.ecl_float_vars[3];
    if (id == -9918) return EnemyManager::GetInstance()->data.ecl_float_vars[4];
    if (id == -9917) return EnemyManager::GetInstance()->data.ecl_float_vars[5];
    if (id == -9916) return EnemyManager::GetInstance()->data.ecl_float_vars[6];
    if (id == -9915) return EnemyManager::GetInstance()->data.ecl_float_vars[7];
    if (id == -9914) return enemyId;
    if (id == -9913) return Globals::get()->photoCount;
    if (id == -9912) return Globals::get()->DS3;
    if (id == -9911) return EnemyManager::GetInstance()->EnmFind(EnemyManager::GetInstance()->data.boss_ids[0])->enemy.final_pos.angle;
    if (id == -9910) return EnemyManager::GetInstance()->EnmFind(EnemyManager::GetInstance()->data.boss_ids[0])->enemy.final_pos.speed;
...
float Enemy::checkVarF(float id)
{
    ...
    if (id == -9988.f) return context.currentContext->time;
    if (id == -9987.f) return Random::Floatm11();
    if (id == -9986.f) return Globals::get()->timeout;
    if (id == -9985.f) return enemy.ecl_int_vars[0];
    if (id == -9984.f) return enemy.ecl_int_vars[1];
    if (id == -9983.f) return enemy.ecl_int_vars[2];
    if (id == -9982.f) return enemy.ecl_int_vars[3];
    ...
    if (id == -9961.f) return enemy.anmIds[0].val;
    if (id == -9960.f) return Globals::get()->rank;
    if (id == -9959.f) return Globals::get()->difficulty;
    if (id == -9958.f) return enemy.final_pos.angle;
    if (id == -9957.f) return true;
    ...
    if (id == -9954.f) return enemy.life.current;
    if (id == -9953.f) return Globals::get()->difficulty == 0;
    if (id == -9952.f) return Globals::get()->difficulty == 1;
    if (id == -9951.f) return Globals::get()->difficulty == 2;
    if (id == -9950.f) return Globals::get()->difficulty == 3;
    if (id == -9949.f) return EnemyManager::GetInstance()->data.miss_count;
    if (id == -9948.f) return EnemyManager::GetInstance()->data.bomb_count;
    if (id == -9947.f) return EnemyManager::GetInstance()->data.can_still_capture_spell;
    if (id == -9946.f) return EnemyManager::GetInstance()->enemyCount;
    if (id == -9945.f) return Globals::get()->shottype;
    ...
    if (id == -9943.f) return EnemyManager::GetInstance()->EnmFind(EnemyManager::GetInstance()->data.boss_ids[0])->enemy.ecl_int_vars[0];
    if (id == -9942.f) return EnemyManager::GetInstance()->EnmFind(EnemyManager::GetInstance()->data.boss_ids[0])->enemy.ecl_int_vars[1];
    if (id == -9941.f) return EnemyManager::GetInstance()->EnmFind(EnemyManager::GetInstance()->data.boss_ids[0])->enemy.ecl_int_vars[2];
    if (id == -9940.f) return EnemyManager::GetInstance()->EnmFind(EnemyManager::GetInstance()->data.boss_ids[0])->enemy.ecl_int_vars[3];
    ...
    if (id == -9931.f) return EnemyManager::GetInstance()->data.last_enemy_id;
    if (id == -9930.f) return Globals::get()->power;
    if (id == -9929.f) return Globals::get()->DS1;
    if (id == -9928.f) return Globals::get()->DS2;
    if (id == -9927.f) return Globals::get()->DS3;
    if (id == -9926.f) return EnemyManager::GetInstance()->data.ecl_int_vars[0];
    if (id == -9925.f) return EnemyManager::GetInstance()->data.ecl_int_vars[1];
    if (id == -9924.f) return EnemyManager::GetInstance()->data.ecl_int_vars[2];
    if (id == -9923.f) return EnemyManager::GetInstance()->data.ecl_int_vars[3];
    ...
    if (id == -9914.f) return enemyId;
    if (id == -9913.f) return Globals::get()->photoCount;
    if (id == -9912.f) return Globals::get()->DS3;
    ...
    if (id == -9909.f) return parentEnemyId;
    if (id == -9908.f) return EnemyManager::GetInstance()->killableEnemyCount();
    if (id == -9907.f) return Globals::get()->spellID;
    if (id == -9906.f) return enemy.abs_pos.flags;
    if (id == -9905.f) return Globals::get()->chapter;
    if (id == -9904.f) return Globals::get()->miss_count_total;
    if (id == -9903.f) return Globals::get()->hyper;
    if (id == -9902.f) return Globals::get()->graze_recent;
    if (id == -9901.f) return Globals::get()->goasts;
    if (id == -9900.f) return Globals::get()->hyperdying;
    if (id == -9899.f) return Globals::get()->achievement_mode;
