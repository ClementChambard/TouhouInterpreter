...
ScreenEffect::ScreenEffect(int _type, int _time, int colorRGB_maybe, int color, int param_6, int on_draw_prio) {
    switch (_type) {
    case 0:
...
int ScreenEffect::f_on_tick_2() {
    ...
    time++;
    if (time < field_0x18) {
        float amt = ((float)(field_0x20 - field_0x1c) * time.current_f) / (float)field_0x18 + (float)field_0x1c;
        int r = rand() % 3;
        if (r == 0) {
...
#define A(col) (col >> 0x18)
        alpha = ns::max(A(field_0x20) - A(field_0x20) * time.current_f / field_0x18, 0.f);
#undef A
    } else {
        field_0x1c--;
        alpha = 0;
...
int ScreenEffect::f_on_tick_6() {
    ...
    if (time < field_0x1c) {
        ...
        intensity = (field_0x20 + field_0x1c + field_0x24 - time.current_f) / field_0x24 * field_0x18;
    }
    int r = rand() % 3;
    if (r == 0) {
        SUPERVISOR.cameras[1].screen_shake.x = 0.0;
...
int ScreenEffect::f_on_draw_1() {
    anm::draw_rect_col({0, 0, anm::BACK_BUFFER_SIZE.x, anm::BACK_BUFFER_SIZE.y}, alpha, field_0x1c);
    return 1;
}

int ScreenEffect::f_on_draw_2() {
    anm::draw_rect_col({GAME_REGION_X, GAME_REGION_Y, GAME_REGION_WIDTH + GAME_REGION_X, GAME_REGION_HEIGHT + GAME_REGION_Y}, alpha, field_0x20);
    return 1;
}
