...
int LaserLine::initialize(void *arg) {
  ...
  LASER_MANAGER_PTR->bullet_anm->copyFromLoaded(
      &vm2, inner.bullet_color + LASER_DATA["spawn_anm_first"].asInt());
  ...
  if ((bullet_type < 18) || (bullet_type == 38)) {
    LASER_MANAGER_PTR->bullet_anm->copyFromLoaded(&vm3, bullet_color + 0x5b);
    vm3.parent_vm = nullptr;
    vm3.__root_vm__or_maybe_not = nullptr;
    vm3.update();
    vm3.bitflags.blendmode = 1;
  } else {
    LASER_MANAGER_PTR->bullet_anm->copyFromLoaded(&vm3, bullet_color + 0x53);
    vm3.parent_vm = nullptr;
    vm3.__root_vm__or_maybe_not = nullptr;
    vm3.update();
  }
  ...
  __field_7c__sometimes_0p01_or_0f =
      (inner.laser_new_arg_2 < inner.laser_new_arg_1) ? 0.01f : 0.f;
...
int LaserLine::on_tick() {
  ...
  do {
    ...
    if (flags & 4) {
      if (ex_accel.timer >= ex_accel.duration) {
        ...
        if (ns::abs(laser_speed.x) > 0.0001 ||
            ns::abs(laser_speed.y) > 0.0001) {
          speed = ns::point_distance(0, 0, laser_speed.x, laser_speed.y);
  ...
  if (inner.laser_new_arg_2 <= laser_inf_current_length) {
    __field_7c__sometimes_0p01_or_0f =
        speed * GAME_SPEED + __field_7c__sometimes_0p01_or_0f;
    laser_offset += laser_speed * GAME_SPEED;
    if ((0.0 < inner.laser_new_arg_3) &&
        (inner.laser_new_arg_3 <
         __field_7c__sometimes_0p01_or_0f + laser_inf_current_length)) {
      laser_inf_current_length =
          inner.laser_new_arg_3 - __field_7c__sometimes_0p01_or_0f;
  ...
  if (inv_timer <= 0) {
    if (ex_offscreen__timer > 0)
      ...
      ns::vec2 b = ns::lengthdir_vec(angle, laser_inf_current_length);
      if ((laser_offset.x + laser_st_width <= -192.0 ||
           192.0 <= laser_offset.x - laser_st_width ||
           laser_offset.y + laser_st_width <= 0.0 ||
           448.0 <= laser_offset.y - laser_st_width) &&
          (laser_offset.x + b.x + laser_st_width <= -192.0 ||
           192.0 <= laser_offset.x + b.x - laser_st_width ||
           laser_offset.y + b.y + laser_st_width <= 0.0 ||
           448.0 <= laser_offset.y + b.y - laser_st_width)) {
        return 1;
      }
    }
  } else {
  ...
  while (et_ex_index < 17 && inner.et_ex[et_ex_index].type &&
    ...
    if (inner.et_ex[et_ex_index].type == 4) {
      ...
      if (990.0 <= inner.et_ex[et_ex_index].s)
        ex_accel.angle = ns::point_direction(laser_offset.x, laser_offset.y,
                                               PLAYER_PTR->inner.pos.x,
                                               PLAYER_PTR->inner.pos.y);
      ex_accel.timer = 0;
      ex_accel.duration = inner.et_ex[et_ex_index].a;
      ex_accel.vec3_a14 = {
          ns::lengthdir_vec(ex_accel.acceleration, ex_accel.angle), 0.f};
    ...
    if (inner.et_ex[et_ex_index].type == 0x2000) {
      ...
      bs.__vec3_8 = {ns::vec2(laser_offset) +
                         ns::lengthdir_vec(laser_inf_current_length, angle),
                     0.f};
...
int FUN_004038d0(float *xout, float *yout, float lx1, float ly1, float lx2,
  ...
  if (fVar7 * fVar6 <= 0.0) {
    ...
    } else if (ns::max(lx1, lx2) < ns::min(x1, x2) ||
               ns::max(ly1, ly2) < ns::min(y1, y2) ||
               ns::max(x1, x2) < ns::min(lx1, lx2) ||
               ns::max(y1, y2) < ns::min(ly1, ly2)) {
      return 0;
    }
...
int LaserLine::method_50_line() {
  ...
  if (((ex_bounce.for_laser_line & 4) != 0) && (laser_pos2.x < -192.0)) {
    if ((ex_bounce.for_laser_line & 0x10) == 0) {
      ...
      inner.spd_1 = ex_bounce.bounce_speed;
      inner.distance = 0.0;
      ns::angle_normalize(inner.ang_aim);
      allocate_new_laser(0, &inner);
    }
    has_bounced = true;
  }
...
int LaserLine::cancel_as_bomb_rectangle(ns::vec3, ns::vec3, float, int, int) {
  return 0;
}
int LaserLine::cancel_as_bomb_circle(ns::vec3 const &, float, int, int) {
  return 0;
}
    ...
    do {
      if (bullet_type < 18 || bullet_type == 34 || bullet_type == 38) {
        BULLET_MANAGER_PTR->bullet_anm->createEffectPos(
            inner.bullet_color * 2 + 0xd1, 0, p);
      } else {
        if (bullet_type < 32) {
          BULLET_MANAGER_PTR->bullet_anm->createEffectPos(
              inner.bullet_color * 2 + 0x101, 0, p);
        }
        if (bullet_type < 0x22) {
          BULLET_MANAGER_PTR->bullet_anm->createEffectPos(
              inner.bullet_color * 2 + 0x119, 0, p);
        }
      }
      if (((as_bomb && (-192.0 < p.x + 32.0)) &&
           ((p.x - 32.0 < 192.0 &&
