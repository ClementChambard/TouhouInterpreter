...
void collect_point_item(Item *item) {
  ...
  if ((PLAYER_PTR->inner.pos.y <= poc) || (item->state == 3)) {
    int val = ((GLOBALS.inner.CURRENT_PIV / 100 -
                (GLOBALS.inner.CURRENT_PIV / 100) % 10) /
               10) *
              10;
    if (val < 1) {
      val = 10;
    ...
    int val = (((((int)(((((piv / 100) % 10 - (piv / 100)) * 3) >> 0x1f & 3U) +
                        (((piv / 100) % 10 - (piv / 100)) * 3)) >>
                  2) *
                 (PLAYER_PTR->inner.pos.y - poc)) /
                    0x1c2 +
                ((int)((((piv / 100) - (piv / 100) % 10) * 3) +
                       ((((piv / 100) - (piv / 100) % 10) * 3) >> 0x1f & 3U)) >>
                 2)) /
               10) *
              10;
    if (val < 1) {
...
int ItemManager::_on_tick() {
  ...
  for (int i = 0; i < 0x1257; i++) {
    Item *item =
        &(i < 600 ? normal_items : cancel_items)[i < 600 ? i : i - 600];
    if (item->state == 0)
      continue;
    if (item->state == 5) {
      ...
      if (item->intangibility_frames < 0) {
        item->state = 2;
        BULLET_MANAGER_PTR->bullet_anm->copyFromLoaded(
            &item->anm_vm_1,
      ...
      if (item->intangibility_frames < 1) {
        ...
        if (PLAYER_PTR->inner.state != 2 && PLAYER_PTR->inner.state != 4 &&
            PLAYER_PTR->inner.pos.y <
                ((GLOBALS.inner.CHARACTER == 1) ? 0x94 : 0x80)
          ...
          // Movement
          item->position +=
              item->velocity * GAME_SPEED * lolk_item_slowdown_factor;
          ...
          if (item->position.y > 472.0 || ns::abs(item->position.x) >= 200.0) {
            // Destroy
            item->state = 0;
            if (item->node_in_free_list.list_head->next) {
              item->node_in_free_list.next =
                  item->node_in_free_list.list_head->next;
              item->node_in_free_list.list_head->next->node_in_free_list.prev =
                  item;
            }
    ...
    if (PLAYER_PTR->inner.state != 2) {
      if (item->position.x >= PLAYER_PTR->item_collect_box.min_pos.x &&
        ...
        case 4:
          ...
          //         iVar7 = *(int *)(&DAT_004a0d64 +
          //         GLOBALS.inner.lifepiece_related * 4);
          //     } else {
          //         iVar7 =
          //         (&ITEM_ANM_SCRIPT_IDS[0x11].id_1)[GLOBALS.lifepiece_related];
          //     }
      ...
      if ((item->state != 4 && item->state != 3) &&
          (((INPUT_STRUCT.input & 8U) &&
            item->position.x >=
                PLAYER_PTR->item_attract_box_focused.min_pos.x &&
            item->position.y >=
                PLAYER_PTR->item_attract_box_focused.min_pos.y &&
            item->position.x <=
                PLAYER_PTR->item_attract_box_focused.max_pos.x &&
            item->position.y <=
                PLAYER_PTR->item_attract_box_focused.max_pos.y) ||
           (!(INPUT_STRUCT.input & 8) &&
            PLAYER_PTR->item_attract_box_unfocused.min_pos.x <=
                item->position.x &&
            PLAYER_PTR->item_attract_box_unfocused.min_pos.y <=
                item->position.y &&
            item->position.x <=
                PLAYER_PTR->item_attract_box_unfocused.max_pos.x &&
            item->position.y <=
...
Item *ItemManager::spawn_item(int type, ns::vec3 const &pos, float angle,
  ...
  if (0x10 < type - 0x10U) {
    ...
    if (item) {
      ...
      BULLET_MANAGER_PTR->bullet_anm->copyFromLoaded(
          &item->anm_vm_1, ITEM_ANM_SCRIPT_IDS[type]["id_1"].asInt());
      item->anm_vm_1.parent_vm = nullptr;
      item->anm_vm_1.__root_vm__or_maybe_not = nullptr;
      item->anm_vm_1.update();
      BULLET_MANAGER_PTR->bullet_anm->copyFromLoaded(
          &item->anm_vm_2, ITEM_ANM_SCRIPT_IDS[type]["id_2"].asInt());
...
void gen_items_from_et_cancel(ns::vec3 const &pos, int do_create_item) {
  if (do_create_item == 1 && -192.0 < pos.x + 32.0 && pos.x - 32.0 < 192.0 &&
