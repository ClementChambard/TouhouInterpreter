...
int Stage::f_on_tick() {
  ...
  inner.camera.facing_normalized =
      normalize(inner.camera.facing + inner.camera.__rocking_vector_2);
...
update_rest:
  ...
  case 6:
    intensity =
        fmax(0.0, (1.0 - inner.rocking_6_timer.current_f / 0x200) * 80.0);
    inner.camera.__rocking_vector_1.y = 0.0;
    inner.camera.__rocking_vector_1.z = 0.0;
    inner.camera.__rocking_vector_1.x =
        sin((inner.rocking_timer.current % 0x400) * PI / 0x200) * intensity;
...
void CameraSkyInterp_t::start(anm::CameraSky_t const &begin, anm::CameraSky_t const &end,
  ...
  color.start({begin.color.r, begin.color.g, begin.color.b, begin.color.a},
              {end.color.r, end.color.g, end.color.b, end.color.a}, time, mode);
  end_time = time;
}
...
bool shouldBeCulledAt(StdOpener::std_entry_header_t *ent,
  ...
  glm::mat4 world_matrix = glm::translate(glm::mat4(1.f), face_pos);
  glm::mat4 mat = cam.projection_matrix * cam.view_matrix * world_matrix;
  glm::vec3 out_verts[16];
  for (int i = 0; i < 16; i++) {
    out_verts[i] = glm::vec4(in_verts[i], 1.f) * mat;
  }
