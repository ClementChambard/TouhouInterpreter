...
{
    if (!file_exists(file))
        file = STAGE_DATA_TABLE[GLOBALS.inner.STAGE_NUM]["ecl_filename"].asString();
    ...
    std::string logoFile = STAGE_DATA_TABLE[GLOBALS.inner.STAGE_NUM]["stage_logo_anm_filename"].asString();
    if (file_exists(logoFile))
        AnmManager::LoadFile(8, logoFile);

    std::string stdFile = STAGE_DATA_TABLE[GLOBALS.inner.STAGE_NUM]["std_filename"].asString();
